...
Eigen::MatrixXd sobel(Eigen::MatrixXd gray_img, bool is_x_dir) {
    Eigen::MatrixXd filtered_img(gray_img.rows()-2, gray_img.cols());
    
    // TODO: implement filter operation
    std::array<int, 3> filter1;
    std::array<int, 3> filter2;
    if (is_x_dir) {
        filter1 = {1, 2, 1};
        filter2 = {1, 0, -1};
    } else {
    ...
    Eigen::MatrixXd filtered_img2(gray_img.rows()-2, gray_img.cols()-2);

    for (int i = 0; i < filtered_img.rows(); i++) {
        for (int j = 1; j < gray_img.cols() - 1; j++) {
            for (int k = i - 1; k < i + 2; k++) {
                filtered_img2(i, j - 1) += gray_img(i, l) * filter2[l - j + 1];
            }
